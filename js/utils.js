...
let PLANTILLAS = [];
const setPlantillas = (arr) => { PLANTILLAS = Array.isArray(arr) ? arr.slice() : []; };
...
// ── CONFIRMACIÓN in-app (reemplaza al confirm() del navegador). Devuelve Promise<boolean> ──
const confirmDialog = (msg, okLabel = 'Eliminar') => new Promise(resolve => {
  const safe = String(msg).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\n/g,'<br>');
  const ov = document.createElement('div');
  ov.className = 'confirm-overlay';
  ov.innerHTML = `
    <div class="confirm-box">
      <div class="confirm-ic"><svg viewBox="0 0 24 24"><path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg></div>
      <div class="confirm-msg">${safe}</div>
